...
setwd("/home/veit/devel/Bioinformatics/IECHOR_EDAM/bioagentsExtract/")
...
for (i in 1:1300) {
  ...
  # request <- paste("wget 'https://bio.agents/api/agent?page=",i, "&format=xml' -O FulliEchor.xml", sep="")
  # url <- system(request)
  request <- paste("wget 'https://bio.agents/api/agent?page=",i, "&format=json' -O FulliEchor.json", sep="")
  url <- system(request)
  FullReg[[i]] <- fromJSON("FulliEchor.json")$list
...
for (i in 1:length(FullReg)) {
  # AllAgents <- c(AllAgents, FullReg[[i]])
  AllAgents <- rbind(AllAgents, FullReg[[i]][,c("collectionID","accessibility","topic","owner",
...
## Functions!
fdmaps <- NULL
for (i in 1:nrow(AllAgents)) {
  for (j in 1:length(AllAgents$'function'[[i]]$output)) {
    # print(paste("Agent",i))
    entry <- AllAgents$'function'[[i]]$output[j][[1]]
  ...
  for (j in 1:length(AllAgents$'function'[[i]]$input)) {
    # print(paste("Agent",i))
    entry <- AllAgents$'function'[[i]]$input[j][[1]]
    for (k in 1:(length(entry$data)/2)) {
      entryf <- entry$format[[k]]$uri
      if(length(entryf)>0) {
        for (l in 1:length(entryf))
...
# output <- lapply(AllAgents$'function',function(x) lapply(x$output,function(y) 
#   c(grep("format_",unlist(y),value=T),grep("data_",unlist(y),value=T))))
# input <- lapply(AllAgents$'function',function(x) lapply(x$input,function(y) 
#   c(grep("format_",unlist(y),value=T),grep("data_",unlist(y),value=T))))
...
colnames(fullFormats) <- c("Format", "Format label", "from bio.agents","from bio.agents label", 
